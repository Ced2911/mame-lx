...
static bool validate_driver(driver_enumerator &drivlist, game_driver_map &names, game_driver_map &descriptions) {
    ...
    enum {
        NAME_LEN_PARENT = 8, NAME_LEN_CLONE = 16
    };
...
void validate_drivers(emu_options &options, const game_driver *curdriver) {
    ...
    b = a + 1;
    if (b > a) {
        mame_printf_error("UINT8 must be 8 bits\n");
        error = true;
    }

    if (sizeof (INT8) != 1) {
        mame_printf_error("INT8 must be 8 bits\n");
        error = true;
    }
    if (sizeof (UINT8) != 1) {
        mame_printf_error("UINT8 must be 8 bits\n");
    ...
    if (sizeof (INT16) != 2) {
        mame_printf_error("INT16 must be 16 bits\n");
        error = true;
    }
    if (sizeof (UINT16) != 2) {
        mame_printf_error("UINT16 must be 16 bits\n");
        error = true;
    }
    if (sizeof (INT32) != 4) {
        mame_printf_error("INT32 must be 32 bits\n");
        error = true;
    }
    if (sizeof (UINT32) != 4) {
        mame_printf_error("UINT32 must be 32 bits\n");
        error = true;
    }
    if (sizeof (INT64) != 8) {
        mame_printf_error("INT64 must be 64 bits\n");
        error = true;
    }
    if (sizeof (UINT64) != 8) {
        mame_printf_error("UINT64 must be 64 bits\n");
        error = true;
    }
#ifdef PTR64
    if (sizeof (void *) != 8) {
        mame_printf_error("PTR64 flag enabled, but was compiled for 32-bit target\n");
        error = true;
    }
#else
    if (sizeof (void *) != 4) {
        mame_printf_error("PTR64 flag not enabled, but was compiled for 64-bit target\n");
        error = true;
    }
#endif
    lsbtest = 0;
...
#ifdef LSB_FIRST
    if (lsbtest == 0xff00) {
        mame_printf_error("LSB_FIRST specified, but running on a big-endian machine\n");
        error = true;
    }
#else
    if (lsbtest == 0x00ff) {
        mame_printf_error("LSB_FIRST not specified, but running on a little-endian machine\n");
        error = true;
    }
#endif
