...
static void asic3_compute_hold(running_machine &machine)
{
	...
	int mode;
	if (!strcmp(machine.system().name,"orlegend111c") || !strcmp(machine.system().name,"orlegendca"))
...
READ16_HANDLER( pgm_asic3_r )
{
	...
	case 0x00:
		{
			if (!strcmp(space->machine().system().name,"orlegend111c") || !strcmp(space->machine().system().name,"orlegendca"))
				res = (state->m_asic3_latch[0] & 0xf7) | ((input_port_read(space->machine(), "Region") << 3) & 0x08);
			else
				res = (state->m_asic3_latch[0] & 0xf7) | ((2 << 3) & 0x08);
		}
		break;
	...
	case 0x02:
		{
			if (!strcmp(space->machine().system().name,"orlegend111c") || !strcmp(space->machine().system().name,"orlegendca"))
				res = (state->m_asic3_latch[2] & 0x7f) | ((input_port_read(space->machine(), "Region") << 6) & 0x80);
			else
				res = (state->m_asic3_latch[2] & 0x7f) | ((2 << 6) & 0x80);
		}
		break;
	case 0x03:
